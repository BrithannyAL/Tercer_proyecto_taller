...
def cargar_archivos_usuario():
    ...
    try:
        with open("usuario.dat", "tr") as lector:
            ...
            if lectura != '':
                respuesta = lectura[0], lectura[1], lectura[2], lectura[3], lectura[4], lectura[5], lectura[6],lectura[7], lectura[8]
            lectura = eval(lector.readline()[:-1])    
            while (lectura != ''):
                respuesta.insertar(usuario(lectura[0], lectura[1], lectura[2], lectura[3], lectura[4], lectura[5], lectura[6], lectura[7], lectura[8]))
                lectura = eval(lector.readline()[:-1])      
